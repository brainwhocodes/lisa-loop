...
func main() {
	...
	fs := flag.NewFlagSet("lisa", flag.ExitOnError)
...
func handleInitCommand(mode string, projectDir string, maxCalls int, timeout int, verbose bool, backend string, ocSettings openCodeSettings, logFormat string) {
	...
	fmt.Println("🚀 Initializing Lisa project...")
	fmt.Println()
...
func handleStatusCommand(projectPath string) {
	...
	fmt.Println("✅ Valid Lisa Codex project")
...
func runWithMonitor(ctx context.Context, controller *loop.Controller, config loop.Config, verbose bool, explicitMode ...loop.ProjectMode) {
	fmt.Printf("🚀 Starting Lisa Codex with TUI monitoring (max %d calls)...\n", config.MaxCalls)
...
func runHeadless(ctx context.Context, controller *loop.Controller, config loop.Config, verbose bool) {
	fmt.Println("🚀 Starting Lisa Codex in headless mode...")
	...
	case err := <-errCh:
		...
		fmt.Println("\n✅ Lisa Codex loop completed successfully")
	case <-ctx.Done():
		fmt.Println("\n🛑 Lisa Codex stopped by user")
		os.Exit(0)
	}
}
...
func runWithLogs(ctx context.Context, controller *loop.Controller, config loop.Config, verbose bool, logFormat string) {
	...
	logger.Info("Starting Lisa Codex in log mode",
	...
	case err := <-errCh:
		...
		logger.Info("Lisa Codex loop completed successfully")
	case <-ctx.Done():
		logger.Warn("Lisa Codex stopped by user")
		os.Exit(0)
	}
}
...
func printHelp() {
	fmt.Println("Lisa Codex - Autonomous AI Development Loop with Charm TUI")
	fmt.Println("")
	fmt.Println("Usage:")
	fmt.Println("  lisa [command] [options]")
	fmt.Println("")
	fmt.Println("Commands:")
	fmt.Println("  run (default)      Run autonomous development loop")
	fmt.Println("  init               Initialize project and launch TUI")
	fmt.Println("  setup              Create a new Lisa-managed project")
