...
# Mock Codex CLI
export MOCK_CODEX_SUCCESS=true
export MOCK_CODEX_THREAD_ID="test-thread-123"
export MOCK_CODEX_EXIT_CODE=0
export MOCK_CODEX_ARGS=""
export MOCK_CODEX_JSONL_OUTPUT=true

mock_codex() {
    # Record all arguments for validation in tests
    MOCK_CODEX_ARGS="$*"

    if [[ "$MOCK_CODEX_SUCCESS" == "true" ]]; then
        # Check if resume mode is being used
        if [[ "$*" =~ "--resume" ]]; then
            echo "Mock: Resuming Codex thread: $MOCK_CODEX_THREAD_ID"
        else
            echo "Mock: Creating new Codex thread"
        fi

        # Simulate Codex exec output (JSONL format for --json flag)
        if [[ "$MOCK_CODEX_JSONL_OUTPUT" == "true" ]]; then
            # Emit JSONL format: one JSON object per line
            echo '{"event": "thread.started", "thread_id": "'"$MOCK_CODEX_THREAD_ID"'", "timestamp": "2026-01-16T12:00:00Z"}'
            echo '{"event": "message", "type": "text", "content": "Test output from Codex"}'
        else
            # Emit simple JSON format
            echo '{"result": "Test output from Codex", "sessionId": "'"$MOCK_CODEX_THREAD_ID"'"}'
        fi

        return $MOCK_CODEX_EXIT_CODE
    else
...
set_mock_codex_success() { MOCK_CODEX_SUCCESS=true; }
set_mock_codex_failure() { MOCK_CODEX_SUCCESS=false; }
