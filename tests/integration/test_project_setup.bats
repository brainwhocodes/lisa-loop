...
EOF
    ...
    mkdir -p templates/.codex
    cat > templates/.codex/config << 'EOF'
# Codex Configuration
backend=cli
codex_exec_timeout=600
EOF

    cat > templates/.codex/README.md << 'EOF'
# Codex Configuration Directory
EOF

    # Create mock skills
    mkdir -p templates/.codex/skills
    cat > templates/.codex/skills/run-tests << 'EOF'
#!/bin/bash
# Mock run-tests skill
echo "Running tests..."
EOF
    chmod +x templates/.codex/skills/run-tests

    cat > templates/.codex/skills/lint << 'EOF'
#!/bin/bash
# Mock lint skill
echo "Running lint..."
EOF
    chmod +x templates/.codex/skills/lint

    cat > templates/.codex/skills/status-summary << 'EOF'
#!/bin/bash
# Mock status-summary skill
echo "Status summary..."
EOF
    chmod +x templates/.codex/skills/status-summary
}
...
@test "setup.sh copies .codex configuration directory" {
    run bash "$SETUP_SCRIPT" test-project

    assert_success
    # Verify .codex directory exists
    assert_dir_exists "test-project/.codex"

    # Verify .codex/config file exists
    assert_file_exists "test-project/.codex/config"

    # Verify .codex/README.md exists
    assert_file_exists "test-project/.codex/README.md"
}

@test "setup.sh copies .codex/skills directory" {
    run bash "$SETUP_SCRIPT" test-project

    assert_success
    # Verify .codex/skills directory exists
    assert_dir_exists "test-project/.codex/skills"

    # Verify run-tests skill exists
    assert_file_exists "test-project/.codex/skills/run-tests"

    # Verify lint skill exists
    assert_file_exists "test-project/.codex/skills/lint"

    # Verify status-summary skill exists
    assert_file_exists "test-project/.codex/skills/status-summary"
}
...
@test "setup.sh handles missing .codex directory gracefully" {
    # Remove .codex directory entirely
    rm -rf templates/.codex
...
@test "setup.sh custom project has all template files" {
    ...
    assert_file_exists "my-custom-app/.codex/config"
    assert_file_exists "my-custom-app/.codex/README.md"
}
...
@test "setup.sh default project has all required structure" {
    ...
    assert_dir_exists "my-project/.codex"
    ...
    assert_file_exists "my-project/.codex/config"
    assert_file_exists "my-project/.codex/README.md"
}
